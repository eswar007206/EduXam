...
import { BookOpen, Loader2, Mail, Lock, User, Eye, EyeOff, ArrowRight, ArrowLeft } from "lucide-react";
...
export default function TeacherSignupPage() {
  ...
  useEffect(() => {
    if (!isLoading && isAuthenticated && profile?.role === "teacher") {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { error: signUpError } = await signUp(email, password, username, "teacher");
    setLoading(false);
    if (signUpError) {
      ...
      <div className="relative z-10 w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-6 lg:p-8 overflow-y-auto">
        ...
        >
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-6 lg:p-8 shadow-2xl lg:bg-white lg:border-gray-200 lg:backdrop-blur-none lg:shadow-xl">
            ...
            <h1 className="text-2xl font-bold text-white lg:text-[#071952]">Teacher Sign up</h1>
            <p className="text-white/70 lg:text-gray-500 text-sm mt-1.5">
              Create an account to access the teacher portal
            </p>

            {success ? (
              <div className="mt-6 p-4 rounded-xl bg-gray-500/10 border border-black/20/20 lg:bg-gray-50 lg:border-black/20">
                <p className="text-black dark:text-black text-sm font-medium text-center">
            ...
            {!success && (
              <p className="text-center text-xs text-white/50 lg:text-gray-500 mt-4">
                Already have an account? <Link to="/teacherauth" className="text-black font-medium hover:underline">Sign in</Link>
              </p>
            )}
          </div>
        </motion.div>
      ...
      <div className="hidden lg:flex w-1/2 relative items-center justify-center overflow-hidden bg-black/5">
        ...
        <div className="relative z-10 max-w-md px-8 text-center">
          ...
          <p className="text-white/70">
            Create subjects, add questions, start exams, and review student submissions.
          </p>
        </div>
      </div>
    </div>
