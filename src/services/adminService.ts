...
export const QUESTION_CATEGORIES: QuestionCategory[] = [
  { key: "mcq", label: "MCQ", type: "mcq", marks: 1, color: "text-blue-700 dark:text-blue-400", bgColor: "bg-blue-100 dark:bg-blue-900/30" },
  { key: "short", label: "Short Answer", type: "descriptive", marks: 3, color: "text-emerald-700 dark:text-emerald-400", bgColor: "bg-emerald-100 dark:bg-emerald-900/30" },
  { key: "medium", label: "Medium Answer", type: "descriptive", marks: 4, color: "text-amber-700 dark:text-amber-400", bgColor: "bg-amber-100 dark:bg-amber-900/30" },
  { key: "long", label: "Long Answer", type: "descriptive", marks: 5, color: "text-blue-700 dark:text-blue-400", bgColor: "bg-blue-100 dark:bg-blue-900/30" },
  { key: "code", label: "Code", type: "code", marks: 6, color: "text-blue-700 dark:text-blue-400", bgColor: "bg-blue-100 dark:bg-blue-900/30" },
];
...
export async function createSubject(
  ...
  const { data, error } = await supabase
    .from("subjects")
    .insert({ name, slug, department_id: departmentId, created_by: createdBy })
    .select()
    .single();
  if (error) throw error;
...
export async function updateSubject(
  id: string,
  updates: { name?: string; slug?: string; department_id?: string }
): Promise<SubjectRow> {
  const { data, error } = await supabase
    .from("subjects")
    .update(updates)
